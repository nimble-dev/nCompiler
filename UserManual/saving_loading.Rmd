...
value(nc1_obj, "Cv") <- c(1.23, 2.34) # Access to fields works.
value(nc1_obj, "Cv") 
serialized <- serialize_nComp_object(nc1_obj, nComp_serialize) #serialize! 
...
nc1_restored_obj <- deserialize_nComp_object(serialized, nComp_deserialize) #deserialize!
...
nClasses can be saved and loaded between sessions. This parameterization and
its particular behavior is far from final.
...
save_nClass(ncObj = obj, ncDef = nc1, serializeFn = nComp_serialize,
            file = "~/savedObj.rds", package.name = "savedObjPkg", 
            dir = tempdir())
```

The following code runs even in a new session, loading all info about `obj` from
what was saved in the 
...
deserialized <- read_nClass(file = "~/savedObj.rds", 
