...
A type declaration takes a form like the character string `"nVector(type='integer')"` or the code `nVector(type='integer')`. The function `type2symbol` converts a type declaration (and possibly other information) into a type symbol. Since multiples types to compose a symbol table often start as a list of type declarations, there is a function `typeList2symbolTable`.

`type2symbol` takes the following steps:
...
-   `symbolNcppVec` : For an `nCppVec` object
