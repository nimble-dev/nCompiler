...
Prior to any compilation steps below, one needs to run the following and then restart R.
```{r, eval=FALSE}
setup_nCompLocal()
```
This creates a installs a tiny helper package, `nCompLocal`, that needs to be built locally.  We have tried to devise a system to do this on-the-fly and not require re-starting R.  It should be feasible, but currently it has fiddly issues.  Steps involving package building and loading are sometimes taken over for special handling in RStudio, which might be part of the issues.  For now, the setup procedure is to run the above and restart R.
...
## Compiling an `nFunction`

This can be compiled by:
```{r compile-mult2}
# See above on setup step of running setup_nCompLocal() and restarting R.
...
## Compiling an `nClass` with full and/or generic interfaces.

For objects of the compiled class, there are two kinds of interfaces, generic and full. 

- In a generic interface, there isn't an R6 object for every compiled object.  This saves a lot of R processing and memory in large collections of compiled objects.
- In a full interface, there is an R6 object for every compiled object.  The full interface is better for nice use of a small number of objects.
...
obj <- Cadd_class$new()
obj$Cnumber <- 3  ## This automatically stores Cnumber in the C++ object...
obj$Cnumber
obj$add_Cnumber(2:4) ## and automatically calls the C++ method add_Cnumber.
```

```{r compile-add-class-generic}
# We can also make a generic interface (or get it out of the full interface).
Cadd_class_gen <- nCompile(add_class, interfaces = "generic")
obj <- Cadd_class_gen()
