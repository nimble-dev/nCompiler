...
test_that("nCompile direct, package, and writePackage work with nClass interfaces", {
    ...
    expect_false(nc1$isCompiled())
    expect_false(nc1$private$Cpublic_obj$isCompiled())
    #debug(nCompiler:::build_compiled_nClasses)
    #debug(nCompiler:::nCompile_finish_nonpackage)
    #debug(nCompiler:::build_compiled_nClass)
    c1 <- nCompile(nc)
    obj <- c1$new()
    expect_true(obj$isCompiled())
    expect_true(obj$private$Cpublic_obj$isCompiled())
    expect_equal(x1 + x2, obj$add_vectors(x1, x2))
    CppObj <- to_generic_interface(obj)# to_generic_interface(obj)
    expect_equal(x1 + x2, method(CppObj, 'add_vectors')(x1, x2))
    expect_equal(x1 + x2, method(obj, 'add_vectors')(x1, x2))
    rm(CppObj, obj); gc();

    #debug(nCompiler:::WP_writeRinterfaces)
    #debug(nCompiler:::build_compiled_nClasses)
    #debug(nCompiler:::build_compiled_nClass)
...
#    debug(nCompiler:::nCompile)
    ...
    expect_true(obj$private$Cpublic_obj$isCompiled())
    expect_equal(x1 + x2, obj$add_vectors(x1, x2))
    expect_equal(CppObj, to_generic_interface(obj))
    rm(CppObj, obj); gc();
