...
// C++ standard library and CppAD must come before R headers because
// Rinternals.h defines a length() macro that breaks any later use of length().
#include<limits>
#include<string>
#include<time.h>
#include "R.h"
...
// It appears that cases beyond ME_DOMAIN are not used anywhere.
#define ML_ERROR(x, s) {                        \
   if(x > ME_DOMAIN) { \
       const char *msg = ""; \
       switch(x) { \
...
// it appeard R_D_val is not used anywhere
