...
#' @export
#' @param pkgName Character string. The name of the package to be built,
...
buildPackage <- function(pkgName,
  ...
  if (roxygenize) roxygen2::roxygenize(
      package.dir = file.path(dir, pkgName),
      roclets = c("rd")
  )
  ...
  install.packages(file.path(dir, pkgName),
                   lib = lib,
                   repos = NULL,
                   type = "source",
                   quiet = quiet)
  ...
  if(load) {
    ...
    if(!is.na(pos)) {
      message("detaching and unloading previous version of ", pkgName)
      detach(paste0("package:",pkgName), unload=TRUE, character.only=TRUE)
    }
    ok <- require(pkgName, lib.loc = lib, character.only = TRUE)
  }
  ok
}
