...
#' Nimble Derivatives
#' 
#' EXPERIMENTAL Computes the value, gradient, and Hessian of a given  \code{nFunction} method.  The R version is currently unimplemented.
#' 
#' @param nimFxn a call to a \code{nFunction} method with arguments included.
...
#' In a nFunction, \code{eigen} is identical to \code{nimEigen}.  If the matrix is symmetric, a faster and more accurate algorithm will be used to compute the eigendecomposition. Note that non-symmetric matrices can have complex eigenvalues,
#' which are not supported by nCompiler.  If a complex eigenvalue or a complex element of an eigenvector is detected, a warning will be issued and that element will be returned as \code{NaN}.
#' 
#' Additionally, \code{returnType(eigenNimbleList())} can be used within a \code{link{nFunction}} to specify that the function will return a \code{\link{nCompilerList}} generated by the \code{nimEigen} function.  \code{eigenNimbleList()} can also be used to define a nested \code{\link{nimbleList}} element.  See the User Manual for usage examples. 
#' 
#' @return
#' The spectral decomposition of \code{x} is returned as a \code{\link{nCompilerList}} with elements:
#' \itemize{
...
#' }
#' 
#' @seealso  \code{\link{nimSvd}} for singular value decompositions in nCompiler. 
#' 
#' @examples
#' eigenvaluesDemoFunction <- nFunction(
#'    setup = function(){
#'      demoMatrix <- diag(4) + 2
#'    },
#'    run = function(){
...
#' 
#' In a \code{nFunction}, \code{svd} is identical to \code{nimSvd}. 
#'  
#'  \code{returnType(svdNimbleList())} can be used within a \code{link{nFunction}} to specify that the function will return a \code{\link{nCompilerList}} generated by the \code{nimSvd} function.  \code{svdNimbleList()} can also be used to define a nested \code{\link{nimbleList}} element.  See the User Manual for usage examples. 
#' 
#' @return
#'  The singular value decomposition of \code{x} is returned as a \code{\link{nCompilerList}} with elements:
#' \itemize{
