...
mangleArgumentNames <- function(argNames) {
  cpp_keywords <- c('alignas', 'alignof', 'and', 'and_eq', 'asm', 'auto',
                    ...
                    'xor', 'xor_eq')
  cmath_keywords <- c('cos', 'sin', 'tan', 'acos', 'asin', 'atan', 'atan2',
                      'cosh', 'sinh', 'tanh', 'acosh', 'asinh', 'atanh', 'exp',
                      'frexp', 'ldexp', 'log', 'log10', 'modf', 'exp2',
                      'expm1', 'ilogb', 'log1p', 'log2', 'logb', 'scalbn',
                      'scalbln', 'pow', 'sqrt', 'cbrt', 'hypot', 'erf', 'erfc',
                      'tgamma', 'lgamma', 'ceil', 'floor', 'fmod', 'trunc',
                      'round', 'lround', 'llround', 'rint', 'lrint', 'llrint',
                      'nearbyint', 'remainder', 'remquo', 'copysign', 'nan',
                      'nextafter', 'nexttoward', 'fdim', 'fmax', 'fmin',
                      'fabs', 'abs', 'fma', 'fpclassify', 'isfinite', 'isinf',
                      'isnan', 'isnormal', 'signbit', 'isgreater',
                      'isgreaterequal', 'isless', 'islessequal',
                      'islessgreater', 'isunordered', 'math_errhandling',
                      'INFINITY', 'NAN', 'HUGE_VAL', 'HUGE_VALF', 'HUGE_VALL',
                      'MATH_ERRNO', 'MATH_ERREXCEPT', 'FP_FAST_FMA',
                      'FP_FAST_FMAF', 'FP_FAST_FMAL', 'FP_INFINITE', 'FP_NAN',
                      'FP_NORMAL', 'FP_SUBNORMAL', 'FP_ZERO', 'FP_ILOGB0',
                      'FP_ILOGBNAN', 'double_t', 'float_t')
  ans <- character()
  if(length(argNames) > 0) {
    conflicts <- which(argNames %in% c(cpp_keywords, cmath_keywords))
