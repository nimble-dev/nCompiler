...
#' @export
#' @name save_nClass_fromPkg
#' @title Save an nClass object generated by an existing package
save_nClass_fromPkg <- function(ncObj, file, classname, package.name, lib) {
  if (isFALSE(get_nOption("serialize"))) {
    set_nOption("serialize", TRUE)
    on.exit(set_nOption("serialize", FALSE))
  }

  require(package.name, character.only = TRUE, lib.loc = lib)
