  ...
  for(i in seq_along(code$args)) {
    if(code$args[[i]]$isCall) {
      ...
      if(length(code$args[[i]]$assertions) > 0) {
        toInsert <- lapply(code$args[[i]]$assertions, function(x) if(inherits(x, 'exprClass')) x else nParse(x))
        before <- unlist(lapply(toInsert, function(x) {if(x$name == 'after') FALSE else TRUE}))
        if(isTRUE(nOptions('experimentalNewSizeProcessing')))
          code$args[[i]]$assertions <- list()  ## Clear assertions field so it can be used by later compiler stages
