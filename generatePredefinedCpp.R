## This script:
## 1. Reads files in nCompiler/R that have predefined nimbleClass definitions.
...
clear_predefined_files <- function(compileAttributes = TRUE) {
  ...
  dir2 = file.path("nCompiler","inst","include","nimble")
...
getRcppPacket <- function(parsed, className) {
  ...
  RcppPacket <- compileNimbleClass(env[[className]], 
    ...
    if(is.call(thisParsed)) {
      if(deparse(thisParsed[[1]]) == "<-") {
        if(is.call(thisParsed[[3]])) {
          if(deparse(thisParsed[[3]][[1]]) == "nimbleClass") {
...
combinedPacket <- nCompiler:::RcppNimblePacket(
...
nCompiler:::writeCppNimble(combinedPacket, dir = file.path("nCompiler","inst","include","nimble"))
nCompiler:::writeCppNimble(combinedPacket, dir = file.path("nCompiler","src"))
require(Rcpp)
compileAttributes("nCompiler")
